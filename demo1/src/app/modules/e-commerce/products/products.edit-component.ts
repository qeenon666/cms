...
export class ProductseditComponent implements OnInit, OnDestroy {
    ...
    const sb = this.route.paramMap.pipe(
      switchMap(params => {
        ...
        if (this.id || this.id > 0) {
          return this.ProductlistService.getItemById(this.id);
        }
  ...
  edit() {
    const sbUpdate = this.ProductlistService.update(this.product).pipe(
  ...
  create() {
    const sbCreate = this.ProductlistService.add(this.product).pipe(
      tap(() => this.router.navigate(['/ecommerce/products'])),
      catchError((errorMessage) => {
        console.error('UPDATE ERROR', errorMessage);
        return of(this.product);
